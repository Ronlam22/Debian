...
[ -z "$latest" ] && latest="$(dpkg -l 'linux-image-[0-9]*' 2>/dev/null | awk '$1=="ii"{print $2}' | sed -E 's/^linux-image-//' | sort -V | tail -n1 || true)"

if [ -n "${latest:-}" ] && [ "$latest" != "$running" ]; then
  log "检测到新内核：$latest（当前运行：$running）"
  (crontab -l 2>/dev/null || true; echo "@reboot /bin/bash -lc '$SCRIPT_PATH --postreboot' $TAG") | crontab -
  log "准备重启........"
  sleep 3
