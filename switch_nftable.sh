...
chain tproxy-prerouting {
    ...
    ct direction != original return
    ct mark 1 ip protocol udp meta mark set 1 tproxy ip to :$TPROXY_PORT accept
    ct mark 1 ip6 nexthdr  udp meta mark set 1 tproxy ip6 to :$TPROXY_PORT accept
    goto tproxy-proxy
}
...
chain tproxy-prerouting {
    ...
    ct direction != original return
    ct mark 1 ip protocol { tcp, udp } meta mark set 1 tproxy ip to :$TPROXY_PORT accept
    ct mark 1 ip6 nexthdr  { tcp, udp } meta mark set 1 tproxy ip6 to :$TPROXY_PORT accept
    goto tproxy-proxy
}
...
chain tproxy-output {
    ...
    ip6 daddr @telegram_ipv6 meta mark set 1 accept
    ip daddr @remote_dns_ipv4 udp dport 53 meta mark set 1 accept
    ip6 daddr @remote_dns_ipv6 udp dport 53 meta mark set 1 accept
    return
}
}
EOF
...
chain tproxy-output {
    ...
    ip daddr @telegram_ipv4 meta mark set 1 accept
    ip6 daddr @telegram_ipv6 meta mark set 1 accept
    ip daddr @remote_dns_ipv4 tcp dport 53 meta mark set 1 accept
    ip daddr @remote_dns_ipv4 udp dport 53 meta mark set 1 accept
    ip6 daddr @remote_dns_ipv6 tcp dport 53 meta mark set 1 accept
    ip6 daddr @remote_dns_ipv6 udp dport 53 meta mark set 1 accept
    return
}
}
